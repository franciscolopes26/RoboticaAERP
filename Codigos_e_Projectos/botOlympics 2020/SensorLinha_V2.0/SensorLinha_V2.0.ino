...
#define rightMaxSpeed 14 // max speed of the robot
#define leftMaxSpeed 14 // max speed of the robot
#define rightBaseSpeed 10 // this is the speed at which the motors should spin when the robot is perfectly on the line
#define leftBaseSpeed 10  // this is the speed at which the motors should spin when the robot is perfectly on the line
...
void loop() {
  ...
  while (one.readButton() == 0) {
    ...
    //  lastPosition = qtr.readLineBlack(sensors); // get calibrated readings along with the line position, refer to the QTR Sensors Arduino Library for more details on line position.
    int error = lastPosition - 2500;
...
//    motors.setM2Speed(leftMotorSpeed);
   ...
   // one.move(rightMotorSpeed,leftMotorSpeed);
    delay(1000);
